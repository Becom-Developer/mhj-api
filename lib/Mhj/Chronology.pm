...
sub _update {
    ...
    my $set_cols   = [ 'title', 'adyear', 'jayear', ];
    my $where_cols = ['id'];
    my $set_q      = [];
...
sub _insert {
    ...
    my $dbh    = $self->build_dbh;
    my $col    = q{title, adyear, jayear, deleted, created_ts, modified_ts};
    my $values = q{?, ?, ?, ?, ?, ?};
    my @data   = ( $title, $params->{adyear}, $params->{jayear}, 0, $dt, $dt );
