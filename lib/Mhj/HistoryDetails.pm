...
sub _update {
    ...
    my $set_cols   = [ 'chronology_id', 'contents', 'adyear_ts' ];
    my $where_cols = ['id'];
    my $set_q      = [];
...
sub _insert {
    ...
    my $dbh     = $self->build_dbh;
    my $col =
      q{chronology_id, contents, adyear_ts, deleted, created_ts, modified_ts};
    my $values = q{?, ?, ?, ?, ?, ?};
    my @data   = (
        $params->{chronology_id},
        $params->{contents}, $params->{adyear_ts}, 0, $dt, $dt
    );
...
sub _list {
    ...
    my $options = shift @args;
    my $params  = $options->{params};
    my $id      = $params->{chronology_id};
    my $row     = $self->single( 'chronology', ['id'], { id => $id } );
    return $self->error->commit("not exist chronology: $id") if !$row;
    my $rows = $self->rows( 'history_details', ['chronology_id'], $params );
    return +{
        chronology      => $row,
        history_details => $rows,
    };
}
